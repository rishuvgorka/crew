...
df = pd.read_csv(f"{inputFileLocation}svvrLoop.csv")#input services
...
def customSort(sheet):
    ...
    while apex1 < len(sheet) and sheet[apex1][22] == " ":
        apex1 += 1
    while apex2 < len(sheet) and sheet[apex2][3] == " ":
        apex2 += 1
    sheet[:apex1] = sorted(sheet[:apex1], key=lambda x: x[9])
    j = 0
    while(j<=apex1):
        if sheet[apex1][16] == " ":
    ...
    while(i<=apex2):
        if sheet[apex2][22] == " ":
            if sheet[i][22]>sheet[apex2+1][22]:
                sheet.insert(i,sheet[apex2])
                i += 1
                apex2 += 1
                sheet.pop(apex2)
        if sheet[i][22]>sheet[apex2][22]:
            sheet.insert(i,sheet[apex2])
            i += 1
            apex2 += 1
            sheet.pop(apex2)
    ...
    if True:
        ...
        for trainNum, dutySet in trainDuty.items():
            ...
            if trainNum in svvrRake:
                ...
                line.append(dutySet[j][4])
                line.append(dutySet[j][3])
                j += 1
                line.append(trainNum)
...
def printChart(dutiesDict, op):
    ...
    if True:
        ...
        for trainNum, dutySet in trainDuty.items():
            ...
            while i < len(dutySet):
                ...
                if i == len(dutySet) - 1 and dutySet[i][1] == "MKPR":
                    line.append(trainNum)
                    line.append(" ")
                    line.append(" ")
                    line.append(dutySet[-1][2])
    ...
    sheet.sort(key= lambda x:(x[3],x[22]))
    customSort(sheet)
    with open(op, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        # Write header
        header = ["TRAIN NO." ," ", " ", "MKPR", "DUTY NO.", "PVGW DN", "DUTY NO.", "KKDA DN", "DUTY NO." ,"MUPR","DUTY NO.", "SVVR", "REVERSAL FROM", "TRAIN NO.", "SVVR","DUTY NO.", "MUPR", "DUTY NO.","KKDA UP","DUTY NO.", "PVGW UP","DUTY NO.","MKPR", " ", " ", "TRAIN NO."]
        writer.writerow(header)
        for row in sheet:
            writer.writerow(row)
...
printRoster(dutiesDict, f"{outputLocation}svvrRoster.csv")
print("Roster generated and saved in ouFiles")
verification(dutiesDict, f"{outputLocation}svvrVerification.csv")
print("verification file for SVVR loop")
svvrChart(dutiesDict, f"{outputLocation}svvrChart.csv")
print("SVVR Tripchart generated and saved in ouFiles")
